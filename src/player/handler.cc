...
URLLoaderHandler* URLLoaderHandler::Create(pp::Instance* instance, IHttpDataListener* listener,
                                           const std::string& url) {
  return new URLLoaderHandler(instance, listener, url);
}

URLLoaderHandler::URLLoaderHandler(pp::Instance* instance, IHttpDataListener* listener, const std::string& url)
    : instance_(instance),
      url_(url),
...
void URLLoaderHandler::AppendDataBytes(const char* buffer, int32_t num_bytes) {
  ...
  instance_->PostMessage("Video data from http");
  listener_->AppendDataBytes(buffer, num_bytes);
