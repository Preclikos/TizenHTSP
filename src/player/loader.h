...
class URLLoaderHandler {
 public:
  ...
  static URLLoaderHandler* Create(pp::Instance* instance_, IHttpDataListener* listener, const std::string& url);
  // Initiates page (URL) download.
  void Start();

 private:
  URLLoaderHandler(pp::Instance* instance_, IHttpDataListener* listener, const std::string& url);
  ~URLLoaderHandler();
